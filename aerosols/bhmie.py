#> @brief    This file is converted from mie.m, see http://atol.ucsd.edu/scatlib/index.htm
##           Bohren and Huffman originally published the code in their book on light scattering
##
## @src      http://scatterlib.googlecode.com/files/bhmie_herbert_kaiser_july2012.py
...
##
## @param[in] x        Size parameter = k*radius = 2pi/lambda * radius   
##                      (lambda is the wavelength in the medium around the scatterers)
## @param[in] refrel   Refraction index (n in complex form for example:  1.5+0.02*i;
## @param[in] nang     Number of angles for S1 and S2 function in range from 0 to pi/2
##
## @return    S1,S2    Funtion which correspond to the (complex) phase functions
## @return    Qext     Extinction efficiency
## @return    Qsca     Scattering efficiency 
## @return    Qback    Backscatter efficiency
## @return    gsca     Ssymmetry parameter
...
def mie(wvln,nr,ni,r,nang):
    Xm = 2.* pi * r / wvln
    s1,s2,Qe,Qs,_,gg = bhmie( Xm, complex(nr,ni), nang )
    qsct  = Qs * pi * r**2
    qext  = Qe * pi * r**2
    qabs  = qext - qsct
    
    S11   = .5 * ( abs(s2)**2 + abs(s1)**2 )
    theta = linspace(0,pi,len(s1))
    norm  = .5 * trapz( S11*sin(theta), x=theta )
    
    return qsct,qext,qabs,gg,theta,S11/norm
